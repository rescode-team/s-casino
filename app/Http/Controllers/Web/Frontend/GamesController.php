...
//            }
        ...
        public function server(\Illuminate\Http\Request $request, $game)
        {
            ...
            $server = new $object();
	print_r($server->get($request, $game, auth()->user()->id));exit();
        ...
        public function go_tournament(\Illuminate\Http\Request $request) {
            $tournament = \VanguardLTE\Tournament::where('status','=','active')->where('start', '<=', \Carbon\Carbon::now())->where('end', '>=', \Carbon\Carbon::now())->orderBy('end', 'ASC')->get();
            $tournamentGames = \VanguardLTE\TournamentGame::join('games','tournament_games.game_id','games.id')->where('tournament_games.tournament_id','=',$tournament[0]['id'])->get();
            $title='Tournament';
            return view("frontend.Tropicoblack.tournament.list",compact('title','tournament','tournamentGames'));
        }
	public function getJackpot(\Illuminate\Http\Request $request) {
            return \VanguardLTE\JPG::limit(4)->where('shop_id',auth()->user()->shop_id)->orderBy('id',"ASC")->get();
