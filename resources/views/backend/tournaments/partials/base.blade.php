<div class="row">
    <div class="col-md-6">
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="name" value="{{ $edit ? $tournament->name : old('name') }}"
                required @if ($denied) disabled @endif>
        </div>
        <div class="form-group">
            ...
            <div class="input-group date">
                ...
                <input type="text" name="start" id="start" class="form-control pull-right datepicker" required
                    value="{{ $edit ? $tournament->start : old('start') }}"
                    @if ($denied) disabled @endif>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="input-group date">
                ...
                <input type="text" name="end" id="end" class="form-control pull-right datepicker" required
                    value="{{ $edit ? $tournament->end : old('end') }}"
                    @if ($denied) disabled @endif>
            </div>
        </div>
        <div class="form-group">
            <label>@lang('app.type')</label>
            {!! Form::select('type', \VanguardLTE\Tournament::$values['type'], $edit ? $tournament->type : old('type'), [
                'class' => 'form-control',
        ...
        <div class="form-group">
            ...
            @endphp
            {!! Form::select('bet', $bets, $edit ? $tournament->bet : old('bet'), [
                'class' => 'form-control',
                'disabled' => $denied ? true : false,
            ]) !!}
        </div>
        <div class="form-group">
            ...
            @endphp
            {!! Form::select('spins', $spins, $edit ? $tournament->spins : old('spins'), [
                'class' => 'form-control',
                'disabled' => $denied ? true : false,
            ]) !!}
        </div>
        ...
        <div class="form-group">
            ...
            <select class="form-control select2" name="categories[]" multiple="multiple" id="categories"
                style="width: 100%;" @if ($denied) disabled @endif>
                <option value="0"
                    {{ (old('categories') && in_array(0, old('categories'))) || ($edit && in_array(0, $cats)) ? 'selected' : '' }}>
                    All</option>
                @foreach ($categories as $key => $category)
                    <option value="{{ $category->id }}"
                        {{ (old('categories') && in_array($category->id, old('categories'))) ||
                        ($edit && in_array($category->id, $cats))
                            ? 'selected'
                            : '' }}>
        ...
        <div class="form-group">
            ...
            {!! Form::select('games[]', $games, $edit && $tournament->games_selected ? $gams : old('games'), [
                'id' => 'games',
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                <input type="number" step="0.0000001" name="prize[]" class="form-control" required
                                    value="{{ $prize }}" @if ($denied) disabled @endif>
                        ...
                        @foreach ($tournament->prizes as $prize)
                            ...
                            <div class="prize_item">
                                <div class="form-group">
                                    <label>@lang('app.prize') {{ $count }}</label>
                                    <div class="input-group">
                                        <input type="number" step="0.0000001" name="prize[]" class="form-control"
                                            required value="{{ $prize->prize }}"
                                            @if ($denied) disabled @endif>
                                        <div class="input-group-btn">
                                            <button type="button" class="btn btn-info delete_prize">-</button>
                                        </div>
                ...
                @for ($i = 0; $i < 10 - $count; $i++)
                    <div class="prize_item">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                <input type="number" step="0.0000001" name="prize[]" class="form-control" required
                                    value="" @if ($denied) disabled @endif>
                                <div class="input-group-btn">
                                    <button type="button" class="btn btn-info delete_prize">-</button>
                                </div>
                            </div>
