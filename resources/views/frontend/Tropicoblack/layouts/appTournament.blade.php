...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/frontend/Tropicoblack/css/tournament.css" />

    <link rel="stylesheet" type="text/css" href="/frontend/Tropicoblack/css/style-mobile.css" />
...
<body data-type="public" style="overflow:hidden">
    ...
    <div id="content" class="" style="display:none">
        ...
        <div class="sticky-top main-bg">
            ...
            <div class="tournament-container tournament-list">
                <div class="subline medium text-uppercase white-shadow d-flex text-white"> Ongoing Tourneys </div>
                <div class="tournament-main-content owl-carousel owl-theme" style="display:flex">
                    @if (count($tournament))
                        @php
                            $GAME_LENGTH = 0;
                        ...
                        @for ($i = 0; $i < count($tournament); $i++)
                            @php
                                $GAME_LENGTH += count($tournament[$i]['games']);
                            @endphp
                        @endfor
                        @for ($i = 0; $i < count($tournament); $i++)
                            @foreach ($tournament[$i]['games'] as $tKey => $tItem)
                                @if ($CURRENT_GAME_LENGTH % 6 == 0)
                                    <div style="display: flex">
                                @endif
                                <div class="tournament-item">
                                    <div class="tournament-prize-pool">
                                        <span class="tournament-prize-pool-text text-uppercase text-white">Prize
                                            Pool:&nbsp;&nbsp;{{ number_format($tournament[$i]['sum_prizes'], 2, '.', '') }}
                                            {{ Auth::user()->currency }}</span>
                                        <img class="tournament-prize-pool-image"
                                            src="/frontend/Tropicoblack/img/coin.png" />
                                    </div>
                                    <a class="game-link" data-start='{{ $tournament[$i]['start'] }}'
                                        data-end='{{ $tournament[$i]['end'] }}' data-now='{{ date('Y-m-d h:i:s') }}'
                                        data-id='{{ $tournament[$i]['id'] }}'
                                        data-game='{{ \VanguardLTE\Game::where('id', $tItem->game_id)->get('name')[0]['name'] }}'>
                                        <div class="tournament-item-image" draggable="false"
                                            style="background-image:url(/frontend/Tropicoblack/ico/{{ \VanguardLTE\Game::where('id', $tItem->game_id)->get('name')[0]['name'] }}.jpg)">
                                        </div>
                                        <div class="tournament-item-pending"></div>
                                    </a>
                                    <div class="count-bar w-100">
                                        <div class="w-50 d-flex align-items-center flex-column">
                                            <span class="text-uppercase blue bold">Time</span>
                                            <div class="black-display w-100 mr-1 text-center">
                                                <span class="blue bold period"
                                                    data-start='{{ $tournament[$i]['start'] }}'
                                                    data-end='{{ $tournament[$i]['end'] }}'
                                                    data-now='{{ date('Y-m-d h:i:s') }}' style="font-size:21px">
                                                    60 MIN
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="w-50 d-flex align-items-center flex-column">
                                            <span class="text-uppercase blue bold">
                                                Starts in
                                            </span>
                                            <div class="black-display w-100 mr-1 text-center">
                                                <span
                                                    class="count-degital tournament-banner__time-val accent countdown"
                                    ...
                                    <div class="w-100 d-flex flex-column align-items-center">
                                        <span class="text-uppercase blue bold display-label">Mode</span>
                                        <div
                                            ...
                                            <span class="mode-text blue bold text-center">
                                                {{ \VanguardLTE\Tournament::$values['type'][$tournament[$i]['type']] }}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                @if ($CURRENT_GAME_LENGTH % 6 == 5)
                </div>
            @elseif($CURRENT_GAME_LENGTH == $GAME_LENGTH - 1)
            </div>
            @endif
            @php
                $CURRENT_GAME_LENGTH++;
    ...
    @if (count($tournament))
        @for ($i = 0; $i < count($tournament); $i++)
            <div class="tournament-container tournament-game-container"
                id="tournament-game-container-{{ $tournament[$i]['id'] }}" style="display:none">
                <div class="tournament-game-info">
                    <div class="tournament-game-menu">
                        <div class="tournament-game-head-icon"></div>
                        <div class="timer-container to-center-abs text-center">
                            ...
                            <div class="timer dark-bg d-flex flex-column justify-content-center"
                                id="countdown-digital-{{ $tournament[$i]['id'] }}">
                                <div class="countdown-digital" data-id="{{ $tournament[$i]['id'] }}"
                                    data-date="{{ $tournament[$i]['end'] }}">
                                </div>
                            </div>
                        </div>
                        <div class="mode-container to-center-abs text-center">
                            ...
                            <div class="mode dark-bg d-flex flex-column justify-content-center">
                                {{ \VanguardLTE\Tournament::$values['type'][$tournament[$i]['type']] }}</div>
                        </div>
                        <div class="prize-container to-center-abs text-center">
                            <div class="prize-text">prize pool</div>
                            <div class="prize dark-bg2 d-flex flex-column justify-content-center">
                                <div class="prize-coin-text">
                                    <span class="icon-gold"></span>
                                    {{ number_format($tournament[$i]['sum_prizes'], 2, '.', '') }}
                                    {{ auth()->user()->currency }}
                                </div>
                            </div>
                        </div>
                        <div class="separator to-center-abs">
                            <input type="hidden" name='tournament_id' id="tournament_id"
                                value="{{ $tournament[$i]['id'] }}" />
                            <input type="hidden" name='tournament_type' id="tournament_type"
                                value=" {{ \VanguardLTE\Tournament::$values['type'][$tournament[$i]['type']] }}" />
                        </div>
                        <div class="user-list-content to-center-abs">
                        </div>
                        <div class="ended-overlay to-center-abs" style="display:none">
                            <div class="icon-ended to-center-abs"></div>
                            <div class="ended-headline to-center-abs"> Well played! </div>
                            <div class="ended-text to-center-abs"> Tourney results are being processed. You
                                will be
                                notified shortly! </div>
                            <div class="ended-button ok-button-green to-center-abs">Okay</div>
                        </div>
                    </div>
                </div>
                <div class="tournament-game-excuse">
                    <iframe class="tournament-game-iframe"
                        id="tournament-game-iframe-{{ $tournament[$i]['id'] }}"></iframe>
                </div>
            </div>
        @endfor
    @endif
    ...
    <div class="footer-menu">
        ...
        <div class="footer-menu-game-kind">
            <div class="footer-menu-game-kind-div">
                <img class="all_btn" src="/frontend/Tropicoblack/img/dashboard/kind/g1btn1.png" />
                <img class="all_btn" style="display: none"
                    src="/frontend/Tropicoblack/img/dashboard/kind/g1btn2.png" />
            </div>
            {{-- <img class="fish_btn" src="/frontend/Tropicoblack/img/dashboard/kind/g2btn1.png" /> --}}
            <div class="footer-menu-game-kind-div">
                <img class="fish_btn" src="/frontend/Tropicoblack/img/dashboard/kind/pro_btn.png" />
                <img class="fish_btn" style="display: none"
                    src="/frontend/Tropicoblack/img/dashboard/kind/pro_active_btn.png" />
            </div>
            {{-- <img class="slot_btn" src="/frontend/Tropicoblack/img/dashboard/kind/g3btn1.png" /> --}}
            <div class="footer-menu-game-kind-div">
                <img class="slot_btn" src="/frontend/Tropicoblack/img/dashboard/kind/my_btn.png" />
                <img class="slot_btn" style="display: none"
                    src="/frontend/Tropicoblack/img/dashboard/kind/my_active_btn.png" />
            </div>
            <div class="footer-menu-game-kind-div">
                <img class="fire_btn" src="/frontend/Tropicoblack/img/dashboard/kind/g4btn1.png" />
                <img class="fire_btn" style="display: none"
                    src="/frontend/Tropicoblack/img/dashboard/kind/g4btn2.png" />
            </div>
    ...
    <script src="/frontend/Tropicoblack/js/tournament.js"></script>
    <script src="/frontend/Tropicoblack/js/fullMobile.js"></script>
    <div id="orientation"></div>
    <div id="mask" class="mask" style="opacity: 0;z-index: -999;">
        <div id="mask_close" style="font-size:22px; font-weight:bold; color: #EEE8AA;"></div>
        <div id="swipe"></div>
    </div>
