...
(function(){
	...
	Phaser.Text.prototype.updateText = function () {
		...
		{
            //NOTE (r.rakhman): [JSCLIENT-2933] Now works as Phaser: no cut
			//but anyway increasing letters spacing while addColor feature use.
			//old code: var lineWidth = this.context.measureText(lines[i]).width;
			var lineWidth = this.measureLine(lines[i]);
			lineWidths[i] = lineWidth;
			maxLineWidth = Math.max(maxLineWidth, lineWidth);
		}

		var width = maxLineWidth + parseInt(this.style.strokeThickness);
		...
		this.context.shadowStroke = this.style.shadowStroke;
		this.context.shadowFill = this.style.shadowFill;
		...
		for (i = 0; i < lines.length; i++)
		{
			...
			linePositionY += this._lineSpacing * i;
	...
	*/
	...
	{
		var drawCurrent = function (x, y, line, type, ctx) {

			var colorsArray = nge.Lib.Helper.jsObjClone( (type === 'stroke') ? ctx.strokeColors : ctx.colors );
			var colorStyle = (type === 'stroke') ? 'strokeStyle' : 'fillStyle';
			var printFunction = (type === 'stroke') ? 'strokeText' : 'fillText';

			if(colorsArray && colorsArray.length > 0){
				if(!colorsArray[0])
					colorsArray[0] = (type === 'stroke') ? ctx.style.stroke : ctx.style.fill;

				//var lineIndex = ctx.text.indexOf(line);
				var lineIndex = ctx.text.replace(/\n|\r/g, "").indexOf(line);
				if(lineIndex > 0){
					if(!colorsArray[lineIndex])
						colorsArray[lineIndex] = colorsArray.slice(0, lineIndex).reverse().find(function(el){
							if (el)
								return true;
					...
					colorsArray = colorsArray.slice(lineIndex, colorsArray.length);
				}

				var objKeys = Object.keys(colorsArray);
				...
				for (var i = 0; i < objKeys.length; i++) {
					...
					var to = (objKeys[i + 1]) ? +objKeys[i + 1] : max;

					if (to > max)
						to = max;

					var color = colorsArray[objKeys[i]];
					ctx.context[colorStyle] = color;
					var currentText = line.slice(from, to);

					ctx.updateShadow(ctx.style.shadowStroke);
					ctx.context[printFunction](currentText, x, y);
					x += ctx.context.measureText(currentText).width;
				...
				ctx.updateShadow(ctx.style.shadowStroke);
				ctx.context[printFunction](line, x, y);
			}
		};
		

		if (this.style.stroke && this.style.strokeThickness)
			drawCurrent(x, y, line, 'stroke', this);

		if (this.style.fill)
			drawCurrent(x, y, line, 'fill', this);
	};
